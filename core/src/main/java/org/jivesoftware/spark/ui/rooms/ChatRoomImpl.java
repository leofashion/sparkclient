...
public class ChatRoomImpl extends ChatRoom {
	...
	public void sendMessage() {
        ...
        ChatInputEditor editor = getChatInputEditor();
        String text = editor.getText();

        if (checkVer(JiveInfo.getVersion(), getParticipantVer())
            && text.indexOf("<fontFormate ") == -1 && IAngelUtil.checkText(text)) {
            // 对方版本支持字体时,增加字体格式
            text = "<fontFormate name=\"" + editor.getFontFamily() + "\" size=\""
                + editor.getFSize() + "\" color=\"" + editor.getFontColor() + "\">" + text
                + "</fontFormate>";
        }
    ...
    private String getParticipantVer() {
        ServerPortIQ spIQ = new ServerPortIQ();
        spIQ.setStanzaId("admin" + "@" + SparkManager.getConnection().getXMPPServiceDomain());
        final UnparsedIQ collector;
        try {
            collector = SparkManager.getConnection()
                .sendIqRequestAndWaitForResponse(spIQ);
            if(org.apache.commons.lang3.StringUtils.equals("serverport",collector.getChildElementName())){
                CharSequence content = collector.getContent();
                try {
                    Document document = DocumentHelper.parseText(content.toString());
                    org.dom4j.Element portel = document.getRootElement().element("version");

                    return portel.getData().toString();
     ...
     * @param localVer
     * @param serverVer
     * @return false:(对方版本小于本版本)
     */
    private static boolean checkVer(String localVer, String serverVer) {
        boolean bol = true;
        if (serverVer == null) {
            return false;
        }
        String[] localArr = localVer.split("\\.");
        String[] serverArr = serverVer.split("\\.");
        if (localArr.length == 2) {
            localArr = getArr3(localArr);
        }
        if (serverArr.length == 2) {
            serverArr = getArr3(serverArr);
        }
        for (int i = 0; i < 3; i++) {
            if (Integer.parseInt(serverArr[i]) == Integer.parseInt(localArr[i])) {
                continue;
            } else {
                if (Integer.parseInt(serverArr[i]) < Integer.parseInt(localArr[i])) {
                    bol = false;
     ...
     * 将数组长度补齐用于做比较
     */
    private static String[] getArr3(String[] temArr) {
