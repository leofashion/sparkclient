...
import com.sweet.chat.CustomPhraseDialog;
import com.sweet.chatFont.FontSetting;
import com.sweet.settings.MsgSendHotkeyManager;
import com.sweet.settings.PhraseManager;
import com.sweet.util.IAngelUtil;
...
public abstract class ChatRoom extends BackgroundPanel implements ActionListener, StanzaListener, DocumentListener, ConnectionListener, FocusListener, ContextMenuListener, ChatFrameToFrontListener {
    ...
    private ChatRoomButton screenCapButton;
	...
	private RolloverButton _phraseButton;
	private RolloverButton _shortcutButton;
    ...
    protected ChatRoom() {
        ...
        _phraseButton = new RolloverButton();
        _phraseButton.setIcon(SparkRes.getImageIcon("PHRASE_IMAGE"));
        _phraseButton.setToolTipText("选择常用语");

        _phraseButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent actionEvent) {
                final JPopupMenu popup = new JPopupMenu();
                ArrayList<String> phrases = PhraseManager.getPhrases();
                for (final String s : phrases) {
                    final JMenuItem temp = new JMenuItem(s);
                    popup.add(temp);
                    temp.addActionListener(new AbstractAction() {
                        private static final long serialVersionUID = 7366122728905744050L;

                        @Override
                        public void actionPerformed(ActionEvent e) {
                            getChatInputEditor().setText(temp.getText());
                        }
                    });
                }
                popup.addSeparator();
                final JMenuItem temp = new JMenuItem("维护我的常用语");
                popup.add(temp);
                temp.addActionListener(new AbstractAction() {
                    private static final long serialVersionUID = 6316597685709354763L;

                    @Override
                    public void actionPerformed(ActionEvent e) {
                        CustomPhraseDialog cpd = new CustomPhraseDialog();
                        cpd.editPhraseMessages();
                    }
                });
                popup.show(getChatInputEditor(), getChatInputEditor().getX(),
                    getChatInputEditor().getY());
            }
        });
        editorBarLeft.add(_phraseButton);

        _shortcutButton = new RolloverButton();
        _shortcutButton.setIcon(SparkRes.getImageIcon("HOTKEY_IMAGE"));
        _shortcutButton.setToolTipText("设置快捷键");

        _shortcutButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent actionEvent) {
                final JPopupMenu popup = new JPopupMenu();
                final JMenuItem enter = new JCheckBoxMenuItem("按Enter发送消息");
                final JMenuItem shiftenter = new JCheckBoxMenuItem("按Ctrl+Enter发送消息");
                enter.setSelected(MsgSendHotkeyManager.isEnter());
                popup.add(enter);
                enter.addActionListener(new AbstractAction() {
                    /**
                     *
                     */
                    private static final long serialVersionUID = 1702871799622137845L;

                    @Override
                    public void actionPerformed(ActionEvent e) {
                        shiftenter.setSelected(!enter.isSelected());
                        MsgSendHotkeyManager.setEnter(enter.isSelected());
                    }
                });
                shiftenter.setSelected(!MsgSendHotkeyManager.isEnter());
                popup.add(shiftenter);
                shiftenter.addActionListener(new AbstractAction() {
                    /**
                     *
                     */
                    private static final long serialVersionUID = 6512341851451330234L;

                    @Override
                    public void actionPerformed(ActionEvent e) {
                        enter.setSelected(!shiftenter.isSelected());
                        MsgSendHotkeyManager.setEnter(!shiftenter.isSelected());
                    }
                });
                popup.show(getChatInputEditor(), getChatInputEditor().getX() + 20,
                    getChatInputEditor().getY());
            }
        });
        editorBarLeft.add(_shortcutButton);
    ...
    static ArrayList<String> list=new ArrayList<String>();//存储处理过的消息
    /**
     * 2020-10-27 liyan
     * 判断消息是否处理过,为解决重复发消息
     * @param message
     * @return
     */
    public boolean isRepetition(Message message){
        if(list.contains(message.getPacketID())){
            list.clear();
            return true;
        }else {
            list.add(message.getPacketID());
            return false;
        }
    }
    ...
    public void insertMessage(Message message) {
        ...
        System.out.println("message:"+message.getPacketID());
        if(isRepetition(message)){//处理过任务不重复处理
            System.out.println("处理过了");
            return;
        }
        System.out.println("进没进来啊============================");
    ...
    public void addToTranscript(Message message, boolean updateDate) {
        ...
        if (msg.indexOf("<fontFormate ") == -1 && IAngelUtil.checkText(msg)) {
            msg = "<fontFormate name=\"" + fontSet.getFontName() + "\" size=\""
                + fontSet.getFontSize() + "\" color=\"" + fontSet.getFontColorStr() + "\">"
                + msg + "</fontFormate>";
        }
    ...
    public static class ChatToolBar extends JPanel {
        ...
        public void addScreenshotButton(ChatRoomButton button) {
            addChatRoomButton(button);
            this.screenCapButton = button;
        }

        public ChatRoomButton getScreenshotButton() {
    ...
    public void addScreenCapButton(ChatRoomButton button) {
        toolbar.addScreenshotButton(button);
    }
