...
public class TranscriptWindow extends ChatArea implements ContextMenuListener
{
    ...
    public TranscriptWindow()
    {
		...
		defaultFont = new Font("宋体", Font.PLAIN, fontSize);
...
//            else
    ...
    public void insertMessage( CharSequence nickname, Message message, Color foreground, Color background )
    {
        ...
        Date sentDateTwo = new Date();
        final ZonedDateTime sentDate;
        if (inf != null) {
            sentDateTwo = inf.getStamp();
            body = "(离线) " + body;
        }
        if (body.indexOf("<fontFormate ") == -1 && IAngelUtil.checkText(body)) {
            FontSetting fontSet = FontSetting.getInstance();
            body = "<fontFormate name=\"" + fontSet.getFontName() + "\" size=\""
                + fontSet.getFontSize() + "\" color=\"" + fontSet.getFontColorStr() + "\">"
                + body + "</fontFormate>";
        }

        if (body.indexOf("<cancelFile ") != -1) {
            try {
                int sp1 = body.indexOf("<cancelFile ");
                int sp2 = 0;
                if (sp1 >= 0) {
                    sp2 = body.indexOf("</cancelFile>", sp1);
                }
                if (sp1 >= 0 && sp2 > 0) {
                    String tmpxml = body.substring(sp1, sp2 + 13);
                    org.dom4j.Document xmldoc = DocumentHelper.parseText(tmpxml);
                    org.dom4j.Element root = xmldoc.getRootElement();
                    String fname = root.attributeValue("fileName");
                    String type = root.attributeValue("type");
                    ReceiveFileTransfer tmptransfer = SparkTransferManager.transFile.get(fname);
                    if (tmptransfer != null) {
                        tmptransfer.cancelTransfer();
                        tmptransfer.setVisible(false);
                        SparkTransferManager.transFile.remove(fname);
                        remove(tmptransfer);
                    }
                    if (type.equals("cancel")) {// 取消
                        body = Res.getString("cancelTrasfer.tip") + "\"" + fname + "\"";
                    } else {// 转离线发送
                        body = Res.getString("cancelTransfer.off.tip") + "\"" + fname + "\"";
                    }
                }
        ...
        try {
            String date = getDate(sentDateTwo);
            ...
            int ofsp1 = body.indexOf("<offlinefile ");
            int ofsp2 = 0;
            if (ofsp1 >= 0) {
                ofsp2 = body.indexOf("</offlinefile>", ofsp1);
            }
            if (ofsp1 >= 0 && ofsp2 > 0) {
                String tmpStr = body.substring(ofsp1, ofsp2 + 14);
...
//                    ReceiveFileTransfer receivingMessageUI = new ReceiveFileTransfer();

//                    addComponent(receivingMessageUI);
//                    receivingMessageUI.acceptOffLineFileTransfer(fname, nickname, fromUser, toUser);
...
//                    add( new MessageEntry( sentDate, isDelayed, nickname.toString(), foreground, body, (Color) UIManager.get( "Message.foreground" ), background ) );
    ...
    public boolean isRepetition(Message message){
        if(list.contains(message.getStanzaId())){
            list.clear();
            return true;
        }else {
            list.add(message.getStanzaId());
            return false;
        }
    }
