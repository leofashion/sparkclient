...
public class ChatRoomTransferDecorator implements KeyListener, FileDropListener, ChatRoomClosingListener, ActionListener {
    ...
    private ChatRoomButton sendDirectoryButton;
    ...
    public ChatRoomTransferDecorator(final ChatRoom chatRoom) {
		...
		sendDirectoryButton = UIComponentRegistry.getButtonFactory().createSendDirectoryButton();
		sendDirectoryButton.setToolTipText("发送文件夹");

		chatRoom.addChatRoomButton(sendDirectoryButton);
		...
		sendDirectoryButton.addActionListener(this);
	...
	private void showDirectoryPicker() {
		SwingWorker worker = new SwingWorker() {
			...
			public void finished() {
				...
				int res = jfc.showOpenDialog(
						SparkManager.getChatManager().getChatContainer().getChatFrame());
				if (res == JFileChooser.APPROVE_OPTION) {
					File dir = jfc.getSelectedFile();
					SparkManager.getTransferManager().setDefaultDirectory(dir.getParentFile());
					SparkManager.getTransferManager().sendDirectory(dir,
