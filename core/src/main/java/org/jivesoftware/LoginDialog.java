...
//import com.sweet.iangelgroup.GroupHelper;
...
public class LoginDialog {
    ...
    protected void afterLogin() {
        ...
        Spark.setServerName(getLoginServer());
        Log.debug("Spark.getServerName():"+Spark.getServerName());
        try {
            LocalPreferences preferences = SettingsManager.getLocalPreferences();
            if (preferences.isAutoLoginOA()) {
                HotKeyUtil.loginOA();
            }
...
//        }
    ...
    private final class LoginPanel extends JPanel implements KeyListener, ActionListener, FocusListener, CallbackHandler {
        ...
        private final JCheckBox autoRunBox = new JCheckBox();
        ...
        LoginPanel() {
            ...
            ResourceUtils.resButton(autoRunBox, Res.getString("checkbox.powerboot"));
            ResourceUtils.resLabel(serverLabel, serverField, Res.getString("label.server.address"));
...
//                serverField.setText("10.62.202.254");
//                serverField.setText("192.168.41.234");
                serverField.setText("10.211.55.4");
            }
            ...
            autoRunBox.setSelected(localPref.getStartOnStartup());
        ...
        private void validateLogin() {
            final SwingWorker loginValidationThread = new SwingWorker() {
                ...
                public Object construct() {
                    ...
                    Log.debug("getServerName():"+getServerName()+" getLoginServer:  "+getLoginServer());
                    ...
                    if (loginSuccessfull) {
                        ...
                        //TODO 自动启动
                        //SettingsManager.setAutoRun(autoRunBox.isSelected());
    ...
    //-------
    // 显示未读消息
    private void showUnReadMessages() throws XmppStringprepException {
        ArrayList<String> jidList = new ArrayList<String>();
        File[] list = new File(SparkManager.getUserDirectory(), "transcripts/")
            .listFiles(new FileFilter() {
                @Override
                public boolean accept(File pathname) {
                    if (pathname.getName().toLowerCase().endsWith("_unread.xml")) {
                        return true;
                    }
                    return false;
                }
        ...
        if (list != null && list.length > 0) {
            for (File file : list) {
                ...
                jidList.add(fileName.substring(0, fileName.lastIndexOf("_")));
            }
        }
        try {
            Thread.sleep(2000);
...
//            for (String unjid : jidList) {
//                if (!GroupHelper.isGroupChat(unjid)) {
//                    ChatManager chatManager = SparkManager.getChatManager();
//                    chatManager.activateChat(unjid,
//                        SparkManager.getUserManager().getUserNicknameFromJID(JidCreate.bareFrom(unjid)));
//                }
//            }
//        }
    }
