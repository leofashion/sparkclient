...
public class SparkToaster {
	...
	private String changeMsg(String msg) {
		...
		if (msg.indexOf("<imsg ") != -1) {
			msg = getText(msg);
		} else if (msg.indexOf("<fontFormate ") != -1) {
			msg = getText(msg);
		} else if (msg.indexOf("<offlinefile ") != -1) {
			// 自动发送文件的消息截取
			int ofsp1 = msg.indexOf("<offlinefile ");
			int ofsp2 = 0;
			if (ofsp1 >= 0) {
				ofsp2 = msg.indexOf("</offlinefile>", ofsp1);
			}
			if (ofsp1 >= 0 && ofsp2 > 0) {//
				msg = msg.substring(ofsp1, ofsp2 + 14);
				msg = changeXML(msg);
			}
		} else if (msg.indexOf("<cancelFile ") != -1) {
			int sp1 = msg.indexOf("<cancelFile ");
			int sp2 = 0;
			if (sp1 >= 0) {
				sp2 = msg.indexOf("</cancelFile>", sp1);
			}
			if (sp1 >= 0 && sp2 > 0) {
				msg = msg.substring(sp1, sp2 + 13);
				msg = changeXML(msg);
			}
		}
		return msg;
	 ...
	 * 
	 * @param str
	 * @return 聊天信息 or 发送任务、通知。。提示信息
	 */
	private String getText(String str) {
		org.dom4j.Document xmldoc;
		try {
			if (str.indexOf("<fontFormate ") > 0) {
				int sp1 = str.indexOf(">", str.indexOf("<fontFormate "));
				int sp2 = str.indexOf("</fontFormate>");
				return str.substring(sp1 + 1, sp2);
			} else if (str.indexOf("<imsg ") != -1) {
				int sp1 = str.indexOf("<imsg ");
				if (sp1 != -1) {
					int sp2 = str.indexOf("</imsg>");
					str = str.substring(sp1, sp2 + 7);
				}
				xmldoc = DocumentHelper.parseText(str);
				org.dom4j.Element root = xmldoc.getRootElement();
				return root.getTextTrim();
			} else if (str.indexOf("<offlinefile ") != -1) {
				// 自动发送文件的消息截取
				int ofsp1 = str.indexOf("<offlinefile ");
				int ofsp2 = 0;
				if (ofsp1 >= 0) {
					ofsp2 = str.indexOf("</offlinefile>", ofsp1);
				}
				if (ofsp1 >= 0 && ofsp2 > 0) {//
					str = str.substring(ofsp1, ofsp2 + 14);
					str = changeXML(str);
				}
			} else if (str.indexOf("<cancelFile ") != -1) {
				int sp1 = str.indexOf("<cancelFile ");
				int sp2 = 0;
				if (sp1 >= 0) {
					sp2 = str.indexOf("</cancelFile>", sp1);
				}
				if (sp1 >= 0 && sp2 > 0) {
					str = str.substring(sp1, sp2 + 13);
					str = changeXML(str);
				}
			} else {
				xmldoc = DocumentHelper.parseText(str);
	 ...
	 * @return 文件：文件名称
	 */
	private String changeXML(String msg) {
		org.dom4j.Document xmldoc;
		try {
			xmldoc = DocumentHelper.parseText(msg);
			org.dom4j.Element root = xmldoc.getRootElement();
			if (msg.indexOf("<offlinefile ") != -1) {
				String fname = root.attributeValue("fname");
				if (fname.indexOf("-") != -1) {
					fname = fname.substring(fname.indexOf("-") + 1);
					msg = Res.getString("message.offlineFile.tip") + fname;
				}
			} else if (msg.indexOf("<cancelFile ") != -1) {
				...
				if (type.equals("cancel")) {// 取消
					msg = Res.getString("cancelTrasfer.tip") + "\"" + fname + "\"";
				} else {// 转离线发送
					msg = Res.getString("cancelTransfer.off.tip") + "\"" + fname + "\"";
				}
			}
		} catch (Exception e) {
			Log.error(e);
